...
(
    ...
    PRIMARY KEY (`id`),
    INDEX           `idx_tbname_refid` (`tb_name`,`ref_id`)
...
(
    ...
    `heart_time`           datetime NULL DEFAULT now(),
    `insert_by`            varchar(100)         DEFAULT NULL COMMENT '操作人',
    `insert_time`          datetime   NOT NULL DEFAULT now() COMMENT '创建时间',
    `update_by`            varchar(100)         DEFAULT NULL COMMENT '操作人',
    `update_time`          datetime   NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
...
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='消费者实例';
...
(
    ...
    `meta_update_time` datetime NULL DEFAULT now() COMMENT '元数据更新时间',
...
(
	...
	`updated_at`	 datetime NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_consumer_group_consumer` (`consumer_id`,`consumer_group_id`),
    INDEX                 `idx_consumer_group_id` (`consumer_group_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8 COMMENT='消息者与消费者组之间的订阅关系';

CREATE TABLE `consumer_group_topic`
(
    `id`                  bigint(20) NOT NULL AUTO_INCREMENT,
    `consumer_group_id`   bigint(20) NOT NULL COMMENT '外键id',
    `consumer_group_name` varchar(100) NOT NULL COMMENT '消费者组名',
    `topic_id`            bigint(20) DEFAULT NULL COMMENT 'topic id',
    `topic_name`          varchar(100) NOT NULL COMMENT 'topic 名称',
    ...
    `thread_size`         int(11) DEFAULT '10' COMMENT '表示每个队列的消费线程池大小',
    ...
    `alarm_emails`        text COMMENT '告警邮件地址',
    `insert_by`           varchar(100)          DEFAULT NULL COMMENT '操作人',
    `insert_time`         datetime    NOT NULL DEFAULT now() COMMENT '创建时间',
    `update_by`           varchar(100)          DEFAULT NULL COMMENT '操作人',
    `update_time`         datetime    NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
	...
	`updated_at`	 datetime NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_group_topic_id` (`consumer_group_id`,`topic_id`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8  COMMENT='消费者订阅的topic';

CREATE TABLE `db_node`
(
    `id`               bigint(20) NOT NULL AUTO_INCREMENT,
    `ip`               varchar(45)           DEFAULT NULL COMMENT '表示数据对应的ip',
    `port`             smallint(11) DEFAULT '0' COMMENT '端口号',
    `db_name`          varchar(100)          DEFAULT NULL COMMENT '数据库名称',
    `db_user_name`     varchar(100)          DEFAULT NULL COMMENT '数据库用户名',
    `db_pass`          varchar(100)          DEFAULT NULL COMMENT '数据库密码',
...
(
    ...
    `heart_time`  datetime NULL DEFAULT now() COMMENT '心跳时间',
    `insert_by`   varchar(100)       DEFAULT NULL COMMENT '操作人',
    `insert_time` datetime NOT NULL DEFAULT now() COMMENT '创建时间',
    `update_by`   varchar(100)       DEFAULT NULL COMMENT '操作人',
    `update_time` datetime NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
    ...
    INDEX           `idx_key_ip` (`key1`,`ip`),
    INDEX           `idx_key` (`key1`)
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8  COMMENT='做master选举使用';

CREATE TABLE `notify_message`
(
    `id`                bigint(20) NOT NULL AUTO_INCREMENT,
    `consumer_group_id` bigint(20) NOT NULL COMMENT '消费者id',
...
(
    ...
    `notify_message_id` bigint(20) NOT NULL COMMENT '外键id',
    `insert_by`         varchar(100)         DEFAULT NULL COMMENT '操作人',
    `insert_time`       datetime   NOT NULL DEFAULT now() COMMENT '创建时间',
    `update_by`         varchar(100)         DEFAULT NULL COMMENT '操作人',
    `update_time`       datetime   NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
...
(
    ...
    `min_id`           bigint(20) NOT NULL DEFAULT '0'COMMENT '最小id',
    `insert_by`        varchar(100)          DEFAULT NULL COMMENT '操作人',
    `insert_time`      datetime    NOT NULL DEFAULT now() COMMENT '创建时间',
    `update_by`        varchar(100)          DEFAULT NULL COMMENT '操作人',
    `update_time`      datetime    NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
    `is_active`        tinyint(4) NOT NULL DEFAULT '0' COMMENT '逻辑删除',
    `lock_version`     bigint(20) NOT NULL DEFAULT '1' COMMENT '版本号',
    `meta_update_time` datetime NULL DEFAULT now() COMMENT '元数据更新时间',
	`created_at` 	 datetime DEFAULT now() COMMENT '创建时间',
	`updated_at`	 datetime NOT NULL DEFAULT now() ON UPDATE now() COMMENT '更新时间',
    PRIMARY KEY (`id`),
    UNIQUE KEY `uk_topic_db` (`topic_name`,`tb_name`,`ip`,`db_name`),
...
(
    ...
    `consumer_id`                bigint(20) DEFAULT '0' COMMENT '外键id',
...
) ENGINE=InnoDB AUTO_INCREMENT=1 DEFAULT CHARSET=utf8  COMMENT='broker实例表';
